...
# ------------------ LIGHTS ------------------ #
def draw_light(light_surface, position, radius, color):
    temp = pygame.Surface((radius*2, radius*2), pygame.SRCALPHA)
    for r in range(radius, 0, -1):
        alpha = int(255 * (r / radius) ** 2)  # quadratic fade
        if alpha > 0:
            pygame.draw.circle(temp, (*color, alpha), (radius, radius), r)
    light_surface.blit(temp, (position[0]-radius, position[1]-radius), special_flags=pygame.BLEND_RGBA_ADD)

# ------------------ MAIN ------------------ #
def main():
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    pygame.display.set_caption("World with Proper Gradient Lights")
    ...
    while running:
        ...
        draw_light(light_surface, (player.x - camera.x, player.y - camera.y), 150, (255, 200, 150))
        draw_light(light_surface, (follower.x - camera.x, follower.y - camera.y), 120, (100, 150, 255))
        draw_light(light_surface, pygame.mouse.get_pos(), 100, (255, 0, 0))
        screen.blit(light_surface, (0,0), special_flags=pygame.BLEND_RGBA_ADD)

        # Instructions
        font = pygame.font.Font(None, 24)
        instructions = ["Arrow/WASD: Move", "S: Save | L: Load", "Mouse shows red light"]
